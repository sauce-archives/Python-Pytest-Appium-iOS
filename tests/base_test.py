...
class BaseTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if not os.getenv("LOCAL_APPIUM", None):
             selenium_uri = "http://%s:%s/wd/hub" % (BaseTest.selenium_host, BaseTest.selenium_port)
        else:
             selenium_uri = "http://%s:%s@%s:%s/wd/hub" % (BaseTest.username,
                                                           BaseTest.access_key,
    ...
    def setup_class(cls):
        ...
        if not os.getenv("LOCAL_APPIUM", None):
            ...
            # default appium port and host
            cls.selenium_host = "localhost"
            cls.selenium_port = "4723"
            cls.app_path = os.path.realpath(__file__ + "/../../resources/TestApp8.4.app.zip")
            cls.upload = False
            cls.app = cls.app_path
